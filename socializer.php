...
<div style="font-size:17px;line-height:188%;margin-left:37px;text-indent:-33px;">
 ...
 Widget settings can be customized below &mdash; keep reading...
...
<label><input type="radio" name="tfw_moboption" value="showmob" <?php if(get_option('tfw_moboption')=="showmob") echo "checked";?>> <span style="letter-spacing:1px;">Show</span> the floating icons even on mobile phones</label><br>
...
<div id="SocializerW">		
	<div id="fb-root"></div><script type="text/javascript">
			...
			<td align="left" valign="bottom" style="border:none;vertical-align:bottom;background-color:transparent;background-image:none;"><?=$followbuttonw ?><a title="Share in top social networks, email, translate, and more!" style="border:none;" href='http://www.socializer.info/share.asp?docurl=http://<?php echo urlencode($_SERVER["HTTP_HOST"]) ?><?php echo urlencode($_SERVER["REQUEST_URI"]); ?>&doctitle=<?php if (function_exists('is_tag') && is_tag()) { $tag; } elseif (is_archive()) { wp_title(''); } elseif (is_search()) { echo 'Search for '.wp_specialchars($s).' - '; } elseif (!(is_404()) && (is_single()) || (is_page())) { wp_title(''); echo ' - '; } elseif (is_404()) { echo 'Not Found - '; } if ( empty( $title ) && is_home() || is_front_page() ) { bloginfo('name'); echo ' - '; bloginfo('description'); } else { echo ' '; bloginfo('name'); } ?>' target="_blank"><img src="<?=$socpath?><?=$imid?>.gif" title="Share in top social networks, email, translate, and more!" style="border:none;"></a></td></tr></table></div>        
<?php
    }
}
